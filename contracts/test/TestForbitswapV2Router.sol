...
contract TestforbitspaceRouter is IforbitspaceSwapCallback {
    ...
    function swapForExact0Multi(
        ...
        pools[0] = poolInput;
        IforbitspacePool(poolOutput).swap(
            recipient,
            false,
            -amount0Out.toInt256(),
    ...
    function forbitspaceSwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid =
                amount0Delta > 0 ? IforbitspacePool(msg.sender).token0() : IforbitspacePool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0 ? amount0Delta : amount1Delta;

            bool zeroForOne = tokenToBePaid == IforbitspacePool(pools[0]).token1();
            IforbitspacePool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
            ...
            if (amount0Delta > 0) {
                IERC20Minimal(IforbitspacePool(msg.sender).token0()).transferFrom(
                    payer,
                    msg.sender,
                    uint256(amount0Delta)
                );
            } else {
                IERC20Minimal(IforbitspacePool(msg.sender).token1()).transferFrom(
                    payer,
                    msg.sender,
